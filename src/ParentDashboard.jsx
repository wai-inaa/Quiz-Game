...
const ParentDashboard = ({ childName, onBack }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen w-[1200px] bg-gradient-to-r from-yellow-400 to-red-400 text-white p-6 relative overflow-hidden">
      ...
      {scores.length === 0 ? (
        <div className="mt-6 flex flex-col items-center">
          <p className="text-2xl font-bold text-white bg-red-600 px-6 py-3 rounded-lg">No quiz results found for {childName}</p>
          <img src={NoDataGif} alt="No Data" className="w-56 h-56 mt-4 " />
        </div>
      ) : (
        <div className="mt-6 w-2/3 bg-white p-6 rounded-xl shadow-2xl text-black border-4 border-yellow-400">
